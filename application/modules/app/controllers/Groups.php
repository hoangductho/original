...
class Groups extends MY_Controller {
    ...
    public function __construct() {
        ...
        $layout_data = [
            'title' => 'Groups',
            'javascript' => array(
                1 => '/assets/js/vienvong/groups.js'
            )
        ];
    ...
    {
        $data['groups'] = $this->MGroups->getAllGroups();
        $data['regions'] = $this->MRegions->getAllRegions();
        $data['regions_list'] = array();

        foreach ($data['regions'] as $key => $value) {
            $data['regions_list'][$value['id']] = $value['name'];
        }

        $this->load->render('Groups/groups', $data);
    }
}